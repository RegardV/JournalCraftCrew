#!/usr/bin/env python3
"""
Enhanced Testing Server for Journal Craft Crew
Local testing environment with proper user flows and data persistence
"""
...
app = FastAPI(
    ...
    description="Enhanced server for local user testing"
)
...
def verify_jwt_token(token: str) -> Optional[dict]:
    """Verify simple JWT-like token for demo"""
...
async def landing_page():
    """Modern landing page with industry-standard authentication UI"""
    html_content = """
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <style>
            ...
            * {
            ...
            .password-strength-bar.weak { width: 25%; background: var(--error-color); }
            .password-strength-bar.fair { width: 50%; background: var(--warning-color); }
    ...
    <body>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-wrapper">
                            <input
                                ...
                                required
                                aria-describedby="register-password-error register-password-feedback"
                        ...
                        <div id="register-password-feedback" class="password-feedback">Use 8+ characters with a mix of letters, numbers & symbols</div>
                        <div id="register-password-error" class="error-message" role="alert"></div>
                    </div>

                    <div class="form-group">
                        <label for="register-profile" class="form-label">Account Type</label>
                        <select
                            id="register-profile"
                            name="profile_type"
                            class="form-select"
                            aria-describedby="register-profile-help"
                        >
                            <option value="personal_journaler">📖 Personal Journaler (10 AI credits)</option>
                            <option value="content_creator">✍️ Content Creator (50 AI credits)</option>
                        </select>
                        <div id="register-profile-help" class="password-feedback">Choose your usage level to determine your AI credits</div>
                    </div>
        ...
        <script>
            ...
            function checkPasswordStrength(password) {
                ...
                switch(strength) {
                    ...
                    case 1:
                        ...
                        feedback.textContent = 'Weak password - add more variety';
                        feedback.style.color = 'var(--error-color)';
                        break;
                    case 2:
                        strengthBar.classList.add('fair');
                        feedback.textContent = 'Fair password - could be stronger';
            ...
            async function handleRegister(event) {
                ...
                try {
                    ...
                    if (response.ok && data.success) {
                        // Success - switch to login tab with success message
                        switchTab('login');
                        ...
                        if (data.detail && data.detail.field) {
                            ...
                            // Show additional requirements if provided
                            if (data.detail.requirements) {
                                showPasswordRequirements(data.detail.requirements);
                            }
            ...
            function showPasswordRequirements(requirements) {
                const feedback = document.getElementById('register-password-feedback');
                feedback.innerHTML = '<strong>Password requirements:</strong><ul style="margin: 0.25rem 0; padding-left: 1.25rem;">' +
...
async def health_check():
    return {
        "status": "healthy",
        "service": "journal-platform-testing",
...
async def generate_journal(request: AIGenerationRequest):
    try:
        ...
        asyncio.create_task(simulate_real_ai_generation(job_id, request.theme, request.title_style))
...
async def get_user_projects():
    projects = list(data_store["projects"].values())
    return {
        "projects": projects,
        "count": len(projects),
        "page": 1,
...
# Background task for AI generation simulation
async def simulate_real_ai_generation(job_id: str, theme: str, title_style: str):
    ...
    for progress, stage in stages:
        if job_id in data_store["ai_jobs"]:
            ...
            data_store["ai_jobs"][job_id]["status"] = "in_progress"
            save_data(data_store)
...
if __name__ == "__main__":
    import uvicorn
    print("🚀 Starting Enhanced Testing Server")
    print("📍 Local Testing Environment")
    print("🌐 Landing Page: http://localhost:8000")
    print("👤 User Registration & Login Available")
    print("📊 Data Persistence: File-based storage")
